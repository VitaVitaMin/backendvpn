...
async def sub_handler(request):
    ...
    vless = (f"vless://{user['uuid']}@{state['ip']}:{VPN_PORT}?encryption=none&security=reality"
             ...
             f"&sid={state['keys']['shortId']}&flow=xtls-rprx-vision#VPN_{user['tg_id']}")
    return web.Response(text=base64.b64encode((vless + "\n").encode()).decode())

@dp.message(Command("start"))
async def cmd_start(message: types.Message):
    kb = InlineKeyboardBuilder()
    kb.row(types.InlineKeyboardButton(text="🚀 Создать мою подписку", callback_data="get_vpn"))
    await message.answer("Добро пожаловать. Нажмите кнопку для генерации персонального ключа.", reply_markup=kb.as_markup())
...
async def handle_get_vpn(callback: types.CallbackQuery):
    ...
    if tg_id in state["users"]:
        ...
        return await callback.message.edit_text(f"Твоя личная ссылка:\n`{link}`", parse_mode="Markdown")

    new_u = {"tg_id": tg_id, "uuid": str(uuid.uuid4()), "token": uuid.uuid4().hex[:16]}
    state["pending"].append(new_u)
    await callback.message.edit_text("⏳ **Генерация персональной подписки...**\nСоздаем уникальный ключ в базе.", parse_mode="Markdown")
    ...
    await callback.message.edit_text("❌ Ошибка синхронизации. Проверьте, запущен ли домашний ПК.")
